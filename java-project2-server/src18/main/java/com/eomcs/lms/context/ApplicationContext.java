...
//        System.out.println(filename);                       // 0번째 글자부터 . 전까지만 남기고 리턴
...
// BoardListCommand는 AbstractCommand를 상속하였고 AbstractCommand는 Command를 구현하였다. 따라서 BoardListCommand는 Command를 구현한 것과 같다.
        if (i == Command.class) {  
          ...
          if (obj != null) { // 제대로 생성했으면 beanContainer에 저장한다.
            ...
            Method getName = clazz.getMethod("getName"); // "/board/list" 등을 가져옴
           
            addBean(          // invoke : getMethod해서 가져온 메서드를 실행하라
...
////////////////////////////////////////////////////////////////////
  ...
  private Object findBean(Class<?> type) {
    ...
    Collection<Object> beans = beanContainer.values(); // 해시테이블의 벨류값 리턴
    for (Object bean : beans) {
      if (type.isInstance(bean))  // 벨류값이 인스턴스이면, 리턴 빈
        return bean;
    }
    return null;
  }
