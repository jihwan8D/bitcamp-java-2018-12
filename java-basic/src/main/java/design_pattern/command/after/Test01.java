...
public class Test01 {
  ...
  public static void main(String[] args) {
    ...
    commandMap.put("/board/upadte", new BoardUpdateCommand());
    ...
    commandMap.put("/member/upadte", new MemberUpdateCommand());
    ...
    while(true) {
      ...
      // 사용자가 입력한 명령어를 가지고 그 명령어를 처리할 객체를 찾는다.
      Command command = commandMap.get(input);

      if (command != null ) {
        // 명령어 사용 규칙에 따라 메서드 호출한다.
        command.execute();
